...
export function GIFGroover() {
  ...
  function Stream(data) {
    ...
    this.getString = function (count) {
      // returns a string from current pos
      var s = ''
      pos = this.pos
      while (count--) {
        s += String.fromCharCode(dat[pos++])
      }
      this.pos = pos
      return s
    }
    this.readSubBlocks = function () {
      ...
      while (size !== 0 && pos < len) {
        count = size = dat[pos++]
        while (count--) {
          data += String.fromCharCode(dat[pos++])
        }
      }
      this.pos = pos
      return data
  ...
  function decodePixels(minSize, data) {
    ...
    while (busy) {
      ...
      if (key === clear) {
        ...
        if (key >= len) {
          dic[len++] = [...dic[last], dic[last][0]]
        } else if (last !== clear) {
          dic[len++] = [...dic[last], dic[key][0]]
        }
        plen = dic[key].length
        for (i = 0; i < plen; i++) {
          pixelBuf[pixelPos++] = dic[key][i]
        }
        if (size < 12 && len === bitVals[size]) {
          size += 1
        }
      }
    }
  ...
  function parse() {
    ...
    if (bitField & 128) {
      ...
      bgColorCSS =
        bg !== undefined ? `rgb(${bg & 255},${(bg >> 8) & 255},${(bg >> 16) & 255})` : `black`
    }
  ...
  function parseImg() {
    ...
    if (frame.localColourTableFlag) {
      frame.localColourTable = createColourTable(1 << ((bitField & 7) + 1))
    }
    if (pixelBufSize !== frame.width * frame.height) {
      pixelBufSize = frame.width * frame.height
    }
    if (transparencyGiven) {
  ...
  function processFrame(frame) {
    ...
    if (!useT) {
      frame.image.ctx.drawImage(lastFrame.image, 0, 0, gifWidth, gifHeight)
    }
    const cData = frame.image.ctx.getImageData(
      frame.leftPos,
      frame.topPos,
      frame.width,
      frame.height
  ...
  function parseExt() {
    ...
    if (blockID === GIF_FILE.GCExt) {
      ...
      comments.push(st.readSubBlocks())
  ...
  function parseBlock() {
    ...
    if (blockId === GIF_FILE.IMAGE) {
      parseImg()
      fireEvent('progress', {
        progress: (((st.pos / st.data.length) * 1000) | 0) / 10,
  ...
  function startLoad(filename) {
    if (gifSrc === undefined) {
      ...
      const message =
        'GIF is limited to a single load. Create a new GIF object to load another gif.'
      error(message)
  ...
  function getFrameAtTime(timeMs) {
    // returns frame that is displayed at timeMs (ms 1/1000th)
    if (timeMs < 0) {
      timeMs = 0
    }
    timeMs %= duration
    var frame = 0
    while (frame < frames.length && timeMs > frames[frame].time + frames[frame].delay) {
      frame += 1
    }
    return frame
  }
    ...
    if (playSpeed === 0) {
      ...
      if (nextFrameTime !== undefined && nextFrame === null) {
        ...
        if (behind < -frameTime / 2) {
          ...
          delayFix = behind
        } // always behind as code take time to execute;
      }
      if (!framesSkipped) {
        if (playSpeed < 0) {
          if (nextFrame !== null) {
  ...
  function _removeEventListener(name) {
    if (events['on' + name] !== undefined) {
      events['on' + name] = undefined
    }
  }
  const gif = {
    ...
    get complete() {
      return complete
    }, // true when loading complete. Does not mean success

    //==============================================================================================================
    // Use to load the gif.
    set src(URL) {
      startLoad(URL)
    }, // load the gif from URL. Note that the gif will only start loading after current execution is complete.
    ...
    get backgroundColor() {
      return bgColorCSS
    }, // returns the background colour as a CSS color value
    get src() {
      return gifSrc
    }, // get the gif URL
    get width() {
    ...
    get allFrames() {
      return frames.map((frame) => frame.image)
    }, // returns array of frames as images (canvas).
    get duration() {
      return duration
    }, // Read only. gif duration in ms (1/1000 second)
    get currentFrame() {
      return currentFrame
    ...
    }, // Read only. Current frame count, during load is number of frames loaded
    get playSpeed() {
      return playSpeed
    }, // play speed 1 normal, 2 twice 0.5 half, -1 reverse etc...
    getFrame(index) {
      // return the frame at index 0. If index is outside range closet first or last frame is returned
    ...
    set currentFrame(index) {
      seekFrame(index)
    }, // seeks to frame index
    set currentTime(time) {
      seek(time)
    }, // seeks to time
    set playSpeed(speed) {
      setPlaySpeed(speed)
    }, // set the play speed. NOTE speed will not take affect if playing until the current frame duration is up.
    ...
    set onload(func) {
      _addEventListener('load', func)
    }, // fires when gif loaded and decode. Will fire if you cancel before all frames are decode.
    set onerror(func) {
      _addEventListener('error', func)
    }, // fires on error
    set onprogress(func) {
      _addEventListener('progress', func)
    }, // fires a load progress event
    set ondecodestart(func) {
      _addEventListener('decodestart', func)
    }, // event fires when gif file content has been read and basic header info is read (width, and height)  and before decoding of frames begins.
